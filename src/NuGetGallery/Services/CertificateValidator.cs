...
namespace NuGetGallery
{
    public sealed class CertificateValidator : ICertificateValidator
    {
        ...
        public void Validate(HttpPostedFileBase file)
        {
            ...
            if (stream == null)
            {
                throw new ArgumentException(Strings.ValidateCertificate_InvalidStream);
            }

            if (!stream.CanSeek)
            {
                throw new ArgumentException(Strings.ValidateCertificate_StreamMustBeSeekable);
            }

            if (file.ContentLength <= 0 || stream.Length <= 0)
            {
                throw new ArgumentException(Strings.ValidateCertificate_InvalidFileLength);
            }
